...
utilisée avec `#include <rmn/fnom.hf>`.

- Ajout d'une interface explicite pour les fonctions de MGI. Il suffit
d'ajouter la ligne `#include <rmn/mgi.hf>` pour l'utiliser.

- Ajout d'un nouveau type de coordonnée vertical pour IP1 (KIND=7) pour la
profondeur en mètres sous le niveau de la mer.

- Le contrôle du niveau de verbosité se fait maintenant a travers le package [App](https://gitlab.science.gc.ca/RPN-SI/App) et 
on peut contrôler le niveau pour les différentes composantes avec les méthodes suivantes:
