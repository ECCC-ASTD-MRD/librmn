...
{
  char *gossip_dir = (char *)malloc(32);
  char *default_dir = (char *)malloc(32);
  
  if ( (gossip_dir = getenv("GOSSIPSERVER")) == NULL )
    {
      printf("Environment variable \"GOSSIPSERVER\" undefined, default channel \"mgi\" will be used\n");
      return default_dir = "mgi";
    }

  if(strlen(gossip_dir) == 0)
    {
...
int connect_to_hostport(char *target2)  /*   %ENTRY%   */
{
     ...
     if(connect(fserver, (struct  sockaddr *)&server, sizeserver) < 0) 
          {
            fprintf(stderr,"Connection to Server Failed, it is not running! \n");
            return(-1);
          }
...
int connect_to_channel_by_name(char *name)  /*   %ENTRY%   */
{
  ...
  return connect_to_channel_by_name_2(get_gossip_dir(), "");
...
void set_stream_timeout(int new_time)
{
  timeout = new_time;
}

int get_stream_timeout()
{
  return timeout;
}
...
void *read_record(int fclient, void *records, int *length, int maxlength, int tokensize)  /*   %ENTRY%   */
{
  ...
  length2 = read_stream(fclient, records2, length1);   /* If length2 = 0 => there was a timeout for reading data */
  if(length2 == 0)
    {
      set_timeout_signal();
      if(records == NULL)
...
#endif
  /* token = strtok(file_name, delimiters); */
  token = strrchr(file_name, '_');
  snprintf(nbuf, sizeof(nbuf) - 1, "%s_gback", token);
#ifdef DEBUG
  fprintf(stderr, " Will Try to rename data file < %s > ***\n", nbuf);
#endif
