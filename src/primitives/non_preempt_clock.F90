...
real(kind = real64) function non_preempt_clock()
    use iso_fortran_env, only: real64
    use rmn_clock_helper, only: the_clock
    implicit none
...
subroutine run_non_preempt_clock
    ...
    integer, external :: id_thread
    real(kind = real64), external :: f_gettimeofday
    external :: micro_sleep
    ...
    is_running = 1
    do while (is_running == 1)
        call micro_sleep(sleep_tick)
        ...
        if(elapsed < sleep_tick * 15.0) the_clock = the_clock + elapsed
        time1 = time2
    end do
...
subroutine do_non_preempt_clock(start_stop)
    ...
    if (start_stop == 1) then  ! start the counting thread
        if (running == 1) return
        is_running = 1
