...
!$omp critical
        if (nBuffer == 0 .or. .not.( &
            ...
            F_Message == msgBuffer(max(1,nBuffer))) ) then
            nBuffer = nBuffer + 1
...
subroutine msg_verbosity(F_msgLevel)
    ...
    external :: msg_set_minMessageLevel
...
subroutine msg_getInfo(F_canWrite_L,F_msgLevelMin,F_msgUnit,F_msgFormat_S)
    ...
    !> Set minimal level of messages to be buffered
subroutine msg_buffer_verbosity(F_msgLevel)
    ...
    !> Get minimal level of messages to be buffered
...
subroutine msg_buffer_reset()
    ...
    !> Flush (print and reset) msg buffer
