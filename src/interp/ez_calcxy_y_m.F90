subroutine ez_calcxy_y_m(wts, idxs, x, y, gdout_lat, gdout_lon, gdout_masque, gdin_lat, gdin_lon, gdin_masque, ni_src, nj_src, ni_dst, nj_dst, num_wts)
    ...
    integer, intent(in) :: ni_src, nj_src, ni_dst, nj_dst, num_wts
    real, intent(in) :: gdin_lon(ni_src, nj_src), gdin_lat(ni_src, nj_src)
    real, intent(in) :: gdout_lon(ni_dst, nj_dst), gdout_lat(ni_dst, nj_dst)
    real, dimension(:, :, :), intent(out) :: wts(ni_dst, nj_dst, num_wts), x(ni_dst, nj_dst), y(ni_dst, nj_dst)
    integer, intent(out) :: idxs(ni_dst, nj_dst, num_wts), gdout_masque(ni_dst, nj_dst)
    integer, intent(in) :: gdin_masque(ni_src, nj_src)
