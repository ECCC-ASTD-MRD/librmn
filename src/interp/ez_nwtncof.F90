...
!> Compute coefficients for the Newtonian form of Lagrange interpolation
subroutine ez_nwtncof(cx, cy, ax, ay, ni, nj, i1, i2, j1, j2, extension)
    use rmn_libc, only: exit
    implicit none

    integer, intent(in) :: ni, nj, i1, i2, j1, j2, extension
    real, dimension(:, :), intent(out) :: cx(ni, 6), cy(j1:j2, 6)
    real, dimension(:), intent(in) :: ax(ni), ay(j1:j2)
