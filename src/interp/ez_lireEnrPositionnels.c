...
void Lire_enrUvercode1(_Grille *gr, float *yy, int32_t nix) {
    int32_t ndiv = (int)yy[2];  // number of LAM grids is 2
    int32_t ni = (int)yy[5];    // ni size of LAM grid 
    int32_t nj = (int)yy[6];    // nj size of LAM grid 
    gr->ni = ni;      // ni size of U grid 
    gr->nj = nj*ndiv; // nj size of U grid 
    float * ax = (float *) malloc(ni*sizeof(float));
    float * ay = (float *) malloc(nj*sizeof(float));
    memcpy(ax, &yy[15], ni * sizeof(float));
    memcpy(ay, &yy[15+ni], nj * sizeof(float));
    int32_t yinsize = 15 + ni + nj;
    gr->nsubgrids = ndiv;
...
void Lire_enrTicTac(
    ...
    const int32_t nixnjx,
    const float * const ay,
    const int32_t niynjy,
    const int32_t ip3,
    const int32_t ip4
) {
    switch (gr->grtyp[0]) {
        ...
        case '#':
            ...
            int32_t offsetx = ip3 - 1;
            int32_t offsety = ip4 - 1;
            for (int32_t  j = 0; j < gr->nj; j++) {
...
int32_t LireEnrPositionnels(_Grille *gr, int32_t iunit, int32_t ip1, int32_t ip2, int32_t ip3, int32_t ip4, int32_t read)
{
    ...
    int32_t tmpip3 = (gr->grtyp[0] == '#') ? -1 : ip3;

    strcpy(etikx, "            ");
    strcpy(etiky, "            ");
    strcpy(typvarx, "  ");
    strcpy(typvary, "  ");
    int32_t intiunit = iunit;
    ...
    if (gr->ni == 0 || gr->ni == -1) {
        //! \bug nix which was uninitialized at this point was assigned to gr->ni! Should this happend AFTER the call to c_fstinf?
        // gr->ni = 0;
    }
    int32_t nix, njx, nkx;
    int Cles_ax = c_fstinf(intiunit, &nix, &njx, &nkx, moins1, etikx, ip1, ip2, tmpip3, typvarx, nomvarx);
    ...
    if (gr->grtyp[0] == 'U') {
        ...
        if (read == 1) {
            ...
            float * yy = (float *)malloc(nix * sizeof(float));
            ier = f77name(fstluk)((uint32_t *)yy, &ier2, &nix, &njx, &nkx);
            // vercode=ig1ref 
            if (ig1ref == 1) Lire_enrUvercode1(gr, yy, nix);
            free(yy);
        }
    }
    ...
    if (gr->grtyp[0] != 'U') {
        ...
        if (gr->nj == 0 || gr->nj == -1) {
            //! \bug nji which was uninitialized at this point was assigned to gr->nj! Should this happend AFTER the call to c_fstinf?
            // gr->nj = 0;
        }
        trouve_y = 0;
        int32_t niy, njy, nky;
        int Cles_ay = c_fstinf(intiunit, &niy, &njy, &nky,moins1, etiky, ip1, ip2, tmpip3, typvary, nomvary);
        int32_t ier1;
        ...
        if (read == 1) {
            ...
            gr->ni_ax = nix;
            gr->nj_ay = njy;
            float * ay = (float *) malloc(niy*njy*sizeof(float));
            ier = f77name(fstluk)((uint32_t *)ay, &ier1, &niy, &njy, &nky);
            float * ax = (float *) malloc(nix*njx*sizeof(float));
