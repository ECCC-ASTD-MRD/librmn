...
/*      fprintf(stderr,"Debug+++ compact_float nbits > 64 EffectivePackedTokenSize=%d bitSizeOfPackedToken=%d opCode=%d\n",
       ...
       ********************************************************************/
    {
      ...
      tempArrayOfInt = (uint32_t *)malloc(sizeof(uint32_t)*elementCount*stride);
     ...
     *********************************************************************************/
  {
     ...
     *                                         *
     ******************************************/
    tempArrayOfInt = (uint32_t *)malloc(sizeof(uint32_t)*(elementCount+2)*stride);
      ...
      if ( hasMissing == 0 ) 
        {
          for ( i = 0; i < intCount*stride; i += stride)
            {
              arrayOfFloat[i] = ( (uint32_t)tempArrayOfInt[i]  *  mulFactor) * 1.0000000000000001 + minFloat;
