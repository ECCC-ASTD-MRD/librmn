...
subroutine dgauss(n, roots, kase)
    ...
    integer, intent(in) :: n
    !> Array containing the zeros of the ordinary Legendre polynomials
    ...
    !> Abramowitz and Stegun "handbook of mathematical functions" Chapter 22 formula 22.16.6.
    ...
    !> A standard identity is used to determine the derivative of polynomial in terms of the values of p(n;x),p(n-1;x).
    ...
    !> Note that the coordinate used is conventionally referred to as mu=cos(theta), running from +1 to -1, for theta from 0 to pie.
    !> The negative roots are  filled by symmetry. For kase=global, all n roots are found, while for
    !> Kase=north/south only the +ve/-ve roots are found, (including 0 if n is odd)  i.e. n/2+mod(n,2) roots.
