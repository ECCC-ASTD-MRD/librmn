...
#include <rmn/fnom.hf>
    ...
    If ( .not.(OPR.eq.1  .or. OPR.eq.5  .or. OPR.eq.7  .or. OPR.eq.8) .and. ETAT.eq.0 ) Return
    ...
    If (abs( OPR ).eq.1) then
        If (OPR.gt.0) then
            ...
            inbr = fstnbr( IUN )
            If (inbr.le. 0) then
                ! Le fichier est vide. Abort ou mode non-controle selon la valeur du drapeau Hard.
                If (Hard) then
                    If (IOPrint) then
                        Write(6,6007)
            ...
            iprm  = fstprm( inf, dateo,deet,npas, NIC,NJC,NKC, nbits,datyp,    ip1,ip20,ip3, typvar,nomvar,etiket, grtyp,ig1,ig2,ig3,ig4, swa,lng,dltf,ubc, extra1,extra2,extra3 )

            ! Allouer l'espace d'une premiere serie de champs en fonction de cette information.
            allocate(sint(ni,nj))
            allocate(cost(ni,nj))
            allocate(bin(ni,nj))
            allocate(pds(ni,nj))
            ...
            inf  = fstinf( IUN,  NIC,NJC,NKC, -1,'CONTROLE ', ip1,ip20,ip3, '+','S/' )
            iluk = fstluk( sint, inf, NIC,NJC,NKC )

            if (NIC.NE.NI .or. NJC.NE.NJ .or. NKC.NE. 1 .or. iluk.lt.0) then
                If (IOPrint) then
                    Write(6,6009)
            ...
            ! Lire COST.
            inf  = fstinf( IUN,  NIC,NJC,NKC, -1,'CONTROLE ', ip1,ip20,ip3, '+','C/' )
            ...
            ! Lire BIN.
            inf  = fstinf( IUN, NIC,NJC,NKC, -1,'CONTROLE ', ip1,ip20,ip3, '+','B/' )
            ...
            ! Lire PDS.
            inf  = fstinf( IUN, NIC,NJC,NKC, -1,'CONTROLE ', ip1,ip20,ip3, '+','W/' )
            ...
            inf  = fstinf( IUN, NIC,NJC,NKC, -1,'CONTROLE ', ip1,ip20,ip3, '+',nomvar )
            If (inf.lt.0) then
                nomvar = 'V/'
                inf  = fstinf( IUN, NIC,NJC,NKC, -1,'CONTROLE ', ip1,ip20,ip3, '+',nomvar )
            End If

            If (inf.lt.0 .or.(nomvar.eq.'VC/' .and. NIC.gt.MaxVarP3*8) .or.(nomvar.eq.'V/'  .and. NIC.gt.MaxVarP3*4)) then
                If (IOPrint) then
                    Write(6,6013)
            ...
            If (mod(MODE,2).eq.1) allocate( somx(SOMNK,NBIP))
            If (MODE.ge.2) allocate( somx2(SOMNK,NBIP) )

            allocate(temp(NIP,SOMNK,NCPU))
            allocate(util(SOMNK,NCPU))
            ...
            If (COMPLET.ne.0) then
                ! Aller lire lire le dernier jeux de champs accumules. Il contient alors le dernier IP2 sauve sur l'unite I/O IUN.
                place = mod( NBIN+1,2 )

                If (mod(MODE,2).eq.1) then
                    inf  = fstinf( IUN,  NIC,NJC,NKC, -1,'MZONXST', ip1,COMPLET,ip3, '+','1/' )
                    iluk = fstluk( WK, inf, NIC,NJC,NKC )

                    If (inf .lt.0 .or. iluk.lt.0    ) then
                        If (IOPrint) then
                    ...
                    ! S'assurer que la premiere dimension de somx est bien impaire.
                    placeX = 1
                    placeY = 1
                    Do  JJ=1,SOMNK
                        Do  II=1,NBIN
                ...
                End If
                If (MODE.ge.2) then
                    inf  = fstinf( IUN,   NIC,NJC,NKC, -1,'MZONXST', ip1,COMPLET,ip3, '+','2/' )
                    iluk = fstluk( WK, inf, NIC,NJC,NKC )
                    ...
                    ! S'assurer que la premiere dimension de somx2 est bien impaire.
                    placeX = 1
                    placeY = 1
                    Do  JJ=1,SOMNK
                        Do  II=1,NBIN
        ...
        Else
            ...
            If (COMPLET.ne.0) then
                ...
                iecr   = fstecr( tabctl, WK, npak,IUN,dateo,deet,npas, nic,njc,1, ip1,ip20,ip3, typvar,nomvar,etiket,grtyp, ig1,ig2,ig3,ig4,datyp, rewrit )
                If (iecr.lt.0) then
                    If (IOPrint) then
                        Write(6,6005)
            ...
            deallocate(sint, cost, bin, pds)
            If (mod(MODE,2).eq.1) deallocate(somx)
            If (MODE.ge.2) deallocate(somx2)
            deallocate(temp, util)
            deallocate(WK)
    ...
    Else If (abs( OPR ).eq.2) then
          If (OPR.gt.0) then
            ...
            ! 3) Decider si on doit sauver quoi que ce soit durant ce pas de temps.
            KOUNT = VAL
            ...
            If (COMPLET.ne.   0   .and. COMPLET.gt. KOUNT ) then
                ! La valeur du pas de temps est plus petite que celle lue sur le fichier IUN.  Il faudra alors tenter de
                ! lire les donnees du pas precedent.
                ip2   = KOUNT-1
                place = mod( NBIN+1,2 )

                If (mod(MODE,2).eq.1) then
                    inf  = fstinf( IUN,  NIC,NJC,NKC, -1,' ', ip1,ip2,ip3, '+','1/' )
                    iluk = fstluk( WK, inf, NIC,NJC,NKC )
                    If (inf .lt.0 .or. iluk.lt.0    ) then
                        If (IOPrint) then
                            Write(6,6020) ip2
                ...
                End If
                If (MODE.ge.2) then
                    inf  = fstinf( IUN,   NIC,NJC,NKC, -1,' ', ip1,ip2,ip3, '+','2/' )
                    iluk = fstluk( WK, inf, NIC,NJC,NKC )
        ...
        Else
            ...
            If (SavPas) then
                ...
                If (mod(MODE,2).eq.1) then
                    ...
                    nomvar = '1/'
                    iecr   = fstecr( WK, somx, npak,IUN,dateo,deet,npas, NBIN,SOMNK,nkc, ip1,ip2,ip3, typvar,nomvar,etiket, grtyp,ig1,ig2,ig3,ig4, datyp,rewrit )
                    If (iecr.lt.0) then
                        If (IOPrint) then
                            Write(6,6017) ip2
                ...
                If (MODE.gt.1) then
                    ...
                    nomvar = '2/'
                    iecr   = fstecr( WK, somx2, npak,IUN,dateo,deet,npas, NBIN,SOMNK,nkc, ip1,ip2,ip3, typvar,nomvar,etiket, grtyp,ig1,ig2,ig3,ig4, datyp,rewrit )
                    If (iecr.lt.0) then
                        If (IOPrint) then
                            Write(6,6018) ip2
                ...
                iecr   = fstecr( tabctl, WK, npak,IUN,dateo,deet,npas, nic,njc,nkc, ip1,ip20,ip3, typvar,nomvar,etiket, grtyp,ig1,ig2,ig3,ig4, datyp,rewrit )
                If (iecr.lt.0) then
                    If (IOPrint) then
                        Write(6,6019) ip2
    ...
    Else If (abs( OPR ).eq.3) then
        ...
        Else
            ...
            ! 1) Transferer l'information de cette tranche dans les accumulateurs (i.e. temp ===> [somx/somx2]).
            ! 2) Liberer la tranche.
            Do  KK=1,NCPU
                If (listej(KK).eq.LocVAL) then
                    listej(KK) = -1
                    ! Determiner le debut et la fin des niveaux contenant des donnees.
                    deb = 1
                    fin = SOMNK

                    Do  JJ=1,SOMNK
                        If (util(JJ,KK).ne.-1) EXIT
                        deb = deb+1
                    End Do
                    ...
                    If (deb.le.fin) then
                        ...
                        Do  II=StartI+1,StartI+NDIMS
                            ...
                            If (mod(MODE,2).eq.1) then
                                Do  JJ=deb,fin
                                    somx (bini,JJ) = somx(bini,JJ) + temp(lo,JJ,KK) * poids
                                End Do
                            End If

                            If (MODE.gt.1) then
                                Do  JJ=deb,fin
                                    somx2(bini,JJ) = somx2(bini,JJ) + temp(lo,JJ,KK) * temp(lo,JJ,KK) * poids
                                End Do
                            End If
                            lo = lo+1
                        End Do
                    ...
                    ! Avant de proceder, remettre a jour la variable ETAT (mais seulement si toutes les tranches verticales sont libres).
                    Do  JJ=1,NCPU
 ...
 6002 Format(' User requests closing of I/O unit ',I3, ' while only unit ',I3,' should be open.')
 ...
 6008 Format(' Unable to read "T/" control table or', ' Incorrect control table sizes :',3I5)
 6009 Format(' Unable to read "S/" sin array or', ' mismatch between expected and real dimensions.')
 6010 Format(' Unable to read "C/" cos array or', ' mismatch between expected and real dimensions.')
 6011 Format(' Unable to read "B/" bin array or', ' mismatch between expected and real dimensions.')
 6012 Format(' Unable to read "W/" weights array or', ' mismatch between expected and real dimensions.')
 6013 Format(' Unable to read "VC/" variable list or', ' mismatch between expected and real dimensions.')
 ...
 6033 Format(' Unable to read "P/" position list or', ' mismatch between expected and real dimensions.')
...
subroutine mzonxst ( Z,NOM,J,NDIM,hhhh,FACF3D,ORD,tttt )
    ...
    Do  II = 1, NVAR
        ...
        if ((listvar(II)(4:4) .ge. '0' .and. listvar(II)(4:4) .le. '9') .or. &
            (listvar(II)(4:4) .ge. 'A' .and. listvar(II)(4:4) .le. 'Z') .or. &
            (listvar(II)(4:4) .ge. 'a' .and. listvar(II)(4:4) .le. 'z')) then

            variab(3:3) = listvar(II)(4:4)
            if ((listvar(II)(5:5) .ge. '0' .and. listvar(II)(5:5) .le. '9' ) .or. &
                (listvar(II)(5:5) .ge. 'A' .and. listvar(II)(5:5) .le. 'Z' ) .or. &
                (listvar(II)(5:5) .ge. 'a' .and. listvar(II)(5:5) .le. 'z' )) then
                variab(4:4) = listvar(II)(5:5)
            endif
        endif

        If (trim(variab) .eq. NOM) EXIT
        JJ = JJ+1
    End Do
    ...
    If (numero.gt.NVAR) then
        ...
        Return
    Else If (.not.SavPas    .and. propvar(numero).eq.1     ) then
        ! C'est une variable echantillonnee et on ne doit pas encore la sauver.
        Return
    Else
        J0 = J+StartJ
        ...
        ! On transfert le contenu du champs Z dans temp selon la valeur trouvee dans le parametre ORD.
        NIC = Min( NDIM, NI )
        ...
        If (ORD.eq.-1) then
            ...
            Do  JJ0=1,NIV
                ...
                Do  II=1,NIC
                    temp(II+StartI,place+JJ0,tranche) = min(  GrosNb, max( -GrosNb, Z(II,JJ)*echant ) )
                End Do
            End Do
        Else If (ORD.eq.0) then
            ! Broadcast scalaire de Z(1,1).
            val11 = min(  GrosNb, max( -GrosNb, Z(1,1)*echant*FACF3D(1,1) ) )
        ...
        Else If (ORD.gt.0 .and. ORD.le.KA) then
            ...
            Do  JJ0=1,NIV
                Do  II=1,NIC
                    temp(II+StartI,place+JJ0,tranche) = min(  GrosNb, max( -GrosNb, Z(II,ORD)*echant ) )
                End Do
            End Do
        Else If (ORD.eq.-2) then
            ...
            Do  JJ0=1,NIV
                JJ = Min( JJ0,Abs( KAJ(tranche) ) )
                Do  II=1,NIC
                    temp(II+StartI,place+JJ0,tranche) = min(  GrosNb, max( -GrosNb, Z(II,JJ)*echant*FACF3D(II,1) ) )
                End Do
            End Do
        Else If (ORD.eq.-3) then
            ...
            Do  JJ0=1,NIV
                JJ = Min( JJ0,Abs( KAJ(tranche) ) )
                Do  II=1,NIC
                    temp(II+StartI,place+JJ0,tranche) = min(  GrosNb, max( -GrosNb, Z(II,JJ)*echant*FACF3D(II,JJ) ) )
                End Do
            End Do
        Else
...
!> Section mise-a-jour de la routine pour les champs vectoriels (Valide seulement dans le mode de controle actif du package).
subroutine MVZNXST ( X,Y,XNOM,YNOM,J,NDIM,FACF3D,ORD,tttt )
    ...
    Do  II=1,NVAR
        ...
        If (trim(variab) .eq. YNOM) EXIT
        JJ     = JJ+1
    End Do

    numeroY = JJ

    If (numeroX.gt.NVAR .or. numeroY.gt.NVAR ) then
        ! Au moins une de ces variables n'est pas dans notre liste.
        Return
    Else If (.not. SavPas .and. (propvar(numeroX).eq.1 .or. propvar(numeroY).eq.1 )) then
        ! Au moins une des variables est une variable echantillonnee et on ne doit pas encore la sauver.
        Return
    Else
        J0 = J+StartJ
        ! Determiner le numero de la tranche.
        ...
        If (propvar(numeroX).eq.1 .or. propvar(numeroY).eq.1 ) then
            echantX = 1.0
            echantY = 1.0
        Else
        ...
        If (ORD.eq.-1) then
            ...
            Do  JJ0 = 1,NIV
                JJ = Min( JJ0,Abs( KAJ(tranche) ) )
                Do  II= 1,NIC
                    temp(II+StartI,placeX+JJ,tranche) = - echantX*X(II,JJ) * SINT(II+StartI,J0) + echantY*Y(II,JJ) * COST(II+StartI,J0)
                    temp(II+StartI,placeY+JJ,tranche) = - echantX*X(II,JJ) * COST(II+StartI,J0) - echantY*Y(II,JJ) * SINT(II+StartI,J0)
                End Do
            End Do
        Else If (ORD.eq.0) then
            ! Broadcast scalaire de X,Y(1,1).
            val11X = ( - echantX*X(1,1) * SINT(1+StartI,J0) + echantY*Y(1,1) * COST(1+StartI,J0) ) * FACF3D(1,1)
            val11Y = ( - echantX*X(1,1) * COST(1+StartI,J0) - echantY*Y(1,1) * SINT(1+StartI,J0) ) * FACF3D(1,1)
            Do  JJ0=1,NIV
                Do  II=1,NIC
        ...
        Else If (ORD.gt.0 .and. ORD.le.KA) then
            ...
            Do  II=1,NIC
                temp(II+StartI,placeX+1,tranche) = - echantX*X(II,ORD) * SINT(II+StartI,J0) + echantY*Y(II,ORD) * COST(II+StartI,J0)
                temp(II+StartI,placeY+1,tranche) = - echantX*X(II,ORD) * COST(II+StartI,J0) - echantY*Y(II,ORD) * SINT(II+StartI,J0)
            End Do

            Do  JJ0=2,NIV
                Do  II=1,NIC
                    temp(II+StartI,placeX+JJ0,tranche) = temp(II+StartI,placeX+1  ,tranche)
                    temp(II+StartI,placeY+JJ0,tranche) = temp(II+StartI,placeY+1  ,tranche)
                End Do
            End Do
        Else If (ORD.eq.-2) then
            ! Copier NIV niveaux. Mode avec normalisation 1D
            Do  JJ0=1,NIV
                JJ = Min( JJ0,Abs( KAJ(tranche) ) )
                Do  II=1,NIC
                    temp(II+StartI,placeX+JJ0,tranche) = ( - X(II,JJ)*echantX * SINT(II+StartI,J0) + Y(II,JJ)*echantY * COST(II+StartI,J0) ) * FACF3D(II,1)
                    temp(II+StartI,placeY+JJ0,tranche) = ( - X(II,JJ)*echantX * COST(II+StartI,J0) - Y(II,JJ)*echantY * SINT(II+StartI,J0) ) * FACF3D(II,1)
                End Do
            End Do
        Else If (ORD.eq.-3) then
            ! Copier NIV niveaux. Mode avec normalisation 2D
            Do  JJ0=1,NIV
                JJ = Min( JJ0,Abs( KAJ(tranche) ) )
                Do  II=1,NIC
                    temp(II+StartI,placeX+JJ0,tranche) = ( - X(II,JJ)*echantX * SINT(II+StartI,J0) + Y(II,JJ)*echantY * COST(II+StartI,J0) ) * FACF3D(II,JJ)
                    temp(II+StartI,placeY+JJ0,tranche) = ( - X(II,JJ)*echantX * COST(II+StartI,J0) - Y(II,JJ)*echantY * SINT(II+StartI,J0) ) * FACF3D(II,JJ)
                End Do
            End Do
        Else
            ! Valeur non-reconnue pour ORD.
...
! Section de modification du KA local KAJ (encore une fois, ceci n'est valide que dans le mode de controle actif du package).
subroutine MZONIV ( J,VAL )
    use mzonxst_mod
    implicit none
