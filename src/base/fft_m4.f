...
C  appended _M4 to the original names to avoid accidents should the new vode
C    be used together with the original code
C  introduced a few top level entry points( setfft4, ffft4, etc ...) that needed
...
C
      IF(IFAX(10).NE.N) CALL SET99_M4(TRIGS,IFAX,N)
      NFAX=IFAX(1)
      NX=N+1
...
C
      ...
      CALL RPASSM_M4(A(IA),A(IA+ILA*INC),WORK(1),WORK(IFAC*ILA+1),TRIGS,
     *    INC,1,JUMP,NX,NVEX,N,IFAC,ILA,IERR)
      GO TO 150
  140 CONTINUE
      CALL RPASSM_M4(WORK(1),WORK(ILA+1),A(IA),A(IA+IFAC*ILA*INC),TRIGS,
...
C
      ...
      CALL QPASSM_M4(A(IA),A(IA+IFAC*ILA*INC),WORK(1),WORK(ILA+1),TRIGS,
     *    INC,1,JUMP,NX,NVEX,N,IFAC,ILA,IERR)
      GO TO 330
  320 CONTINUE
      CALL QPASSM_M4(WORK(1),WORK(IFAC*ILA+1),A(IA),A(IA+ILA*INC),TRIGS,
     *    1,INC,NX,JUMP,NVEX,N,IFAC,ILA,IERR)
  330 CONTINUE
      IF (IERR.NE.0) GO TO 500
      IGO=-IGO
...
C
      DATA SIN36/0.587785252292473D0/
