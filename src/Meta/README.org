...
This metadata is in addition to the previous NOMVAR,TYPVAR,IP,... indexable metadata already available and will garantee backward compatibility with previous FST89 and FST98
It uses a grammar for token/value pair definitions implemented as JSON, and versionned through JSON profiles. 
Management of the profiles and accetep value pair shall be managed by the [[https://wiki.cmc.ec.gc.ca/wiki/Comit%C3%A9_de_gouvernance_des_donn%C3%A9es_du_CMC] Data Governance Commitee (DGC/CGD)]
