...
int missing_value_used() {
    ...
    if (plugmode == -1) {
        ...
        if (text != NULL) {
            Lib_Log(APP_INFO,APP_LIBRMN,"%s: opening plugin library '%s\n",__func__,text);
            ...
            if (handle != NULL) {
                ...
                Lib_Log(APP_WARNING,APP_LIBRMN,"%s: plugin library '%s' not found\n",__func__,text);
            }
        }
...
static int fst_int_encode_missing(
    ...
    if (plug > max) {
        ...
        Lib_Log(APP_WARNING,APP_LIBRMN,"%s: Maximum value >= encoded missing value flag\n",__func__);
    }
    for (int i = 0; i < nElems; i++) {
...
static int fst_uint_encode_missing(
    ...
    if (plug > max) {
        ...
        Lib_Log(APP_WARNING,APP_LIBRMN,"%s: Maximum value %u >= encoded missing value flag %u\n",__func__,max,plug);
    }
    for (int i = 0; i < nElems; i++) {
...
static int fst_ushort_encode_missing(
    ...
    if (plug > max) {
        ...
        Lib_Log(APP_WARNING,APP_LIBRMN,"%s: Maximum value %hu >= encoded missing value flag %hu\n",__func__,max,plug);
    }
    for (int i = 0; i < nElems; i++) {
...
int EncodeMissingValue(
    ...
    Lib_Log(APP_DEBUG,APP_LIBRMN,"%s: %d missing values in %d data values replaced, base datatype=%d\n",__func__,missing,nElems,ldatatype);
