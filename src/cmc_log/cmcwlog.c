...
int c_cmcwlog3(
    ...
    //! [in] Message class, 2 chars max
    const char * const cl,
    //! [in] Message number, 4 digits max
    const int msgno,
    //! [in] Message id, MAXID-1 chars max
    const char * const id,
    //! [in] Message text, MAXTEXT-1 chars max
    ...
    int lmsgno = msgno;
    if (lmsgno > 9999) lmsgno = 9999;
    if (lmsgno < 0) lmsgno = 0;


    // mode_t mask = umask(0);
    // fprintf(stderr,"file=%s, id=%s, msgno=%d, cl=%s, txt=%s, umask=%d\n",filen,id,lmsgno,cl,txt,mask);
    ...
    fprintf(fp, "%s%04i%s%s%s\n", msgcl, lmsgno, msgid, tstamp, msgtxt);
...
int c_cmcwlog2B(
    //! [in] Path of the file into which to write entries
    const char * const filen,
    //! [in] Message class, 2 chars max
    const char * const cl,
    //! [in] Message number, 4 digits max
...
*/
int c_cmcwlog2(
    //! [in] Coma seperated list of file paths into which to write entries
    const char * filen,
    //! [in] Message class, 2 chars max
    const char * const cl,
    //! [in] Message number, 4 digits max
...
int c_cmcwlog(
    ...
    const char * filen = getenv("CMC_LOGFILE_PLUS");
    if (filen == NULL) {
        filen = getenv("CMC_LOGFILE");
        if ( filen == NULL) {
            return 5;
        }
    }
...
int32_t f77name(cmcwlog)(
    //! [in] Message class, 2 chars max
    const char * const fclstr,
    //! [in] Message number, 4 digits max
    const int32_t * const msgno,
    //! [in] Message id, MAXID-1 chars max
    const char * const fidstr,
    //! [in] Message text, MAXTEXT-1 chars max
    const char * const ftxtstr,
    //! [in]
    F2Cl lclstr,
    //! [in]
    F2Cl lidstr,
