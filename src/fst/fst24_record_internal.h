...
static inline void fst_record_set_to_default(fst_record* const record) {
    const int64_t flags = record->do_not_touch.flags;
    const int64_t alloc = record->do_not_touch.alloc;
    void* const data  = record->data;
    *record = default_fst_record;
    record->do_not_touch.flags = flags;
    record->do_not_touch.alloc = alloc;
    record->data  = data;
}
