...
int c_armn_compress32(
    ...
    if (ni < 16 || nj < 16) {
        zlng = -1;
        Lib_Log(APP_ERROR,APP_LIBRMN,"%s: The dimensions of NI and NJ have to be > 16\n",__func__);
        return zlng;
    }
    ...
    if (nbits_needed == 0) {
        ...
        if (lng_exposant > (unsigned int) ni * nj) {
            zlng = -1;
            Lib_Log(APP_WARNING,APP_LIBRMN,"%s: Exponent range too large, original field left uncompressed",__func__);
            return zlng;
        }
    ...
    memcpy(&zstream[sizeof(unsigned int)], &zieee_info, sizeof(unsigned int));
...
int c_armn_uncompress32(
    ...
    unsigned char *signe, *zsigne, code_signe, code_exposant, code_mantisse;
    unsigned char codes;
    unsigned int *cur, nbits_needed_exposant;
    ...
    signe = (unsigned char *) malloc(2 * npts * sizeof(unsigned char));
    if (signe == NULL) {
        Lib_Log(APP_ERROR,APP_LIBRMN,"%s: Failed to allocate memory (%s:%d)",__func__,__FILE__,__LINE__);
        return(-1);
    }
    zsigne = (unsigned char *) malloc(2 * npts * sizeof(unsigned char));
    if (zsigne == NULL) {
...
int c_fstzip32(unsigned int *zfld, unsigned int *fld, int ni, int nj, int nk, int step, int nbits, int remaining_space)
{
    ...
    if (zlng == 0) {
        Lib_Log(APP_WARNING,APP_LIBRMN,"%s: IEEE compressed field is larger than original, keeping original\n",__func__);
        return 0;
    }
