...
! */
subroutine bmf_splitwritey(nom,gni,gnj,nk,time1,time2, &
...
! hgrid, vgrid    Horizontal and vertical grid descriptors
! dtyp            Data type
...
! the second ("y") axis
  ...
  logical fill
  integer size
  integer  RPN_COMM_split, bmf_write2
  external RPN_COMM_split, bmf_write2
  size=dtyp/10/4
  ptwit=loc(vecteur(1,1,1,1))
  fill=.false.
  halox=0
  haloy=0
  ierr=0
  ...
  nullify(p_vecteur)
  ierr= RPN_COMM_split(0,1,gni,minx,maxx,nxl,nxlmax, &
                    halox,nx0,fill)
  if(ierr.ne.0) then
       write(*,*) 'BMF_SPLITWRITEY: error RPN_COMM_split, abort'
       stop
  endif
  do py=0,bmf_npey-1
     ierr= RPN_COMM_split(py,bmf_npey,vgnj_bmf,miny,maxy,nyl,nylmax, &
                      haloy,ny0,fill)
     if(ierr.ne.0) then
          write(*,*) 'BMF_SPLITWRITEY: error RPN_COMM_split, abort'
          stop
