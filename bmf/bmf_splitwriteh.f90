...
! */
subroutine bmf_splitwriteh(nom,minx,maxx,miny,maxy,nk,ni0,&
...
!
  ...
  do px=0,bmf_npex-1
     ierr= RPN_COMM_split(px,bmf_npex,gni,minx2,maxx2,nxl,nxlmax, &
          halox,nx0,fill)
     if(ierr.ne.0) then
        write(*,*) 'BMF_SPLITWRITEH: error RPN_COMM_split, abort'
        stop
     endif
     do py=0,bmf_npey-1
        no_pe=no_pe+1
        ierr= RPN_COMM_split(py,bmf_npey,gnj,miny2,maxy2,nyl,nylmax, &
             haloy,ny0,fill)
...
! On se positionne au dernier point
  ...
  return
end subroutine bmf_splitwriteh
