...
function test_fst24_interface(is_rsf) result(success)
    ...
    block
        ...
        success = create_file(test_file_names(1), .not. is_rsf, test_record % ip2 + 1, test_record % ip3 + 1)   &
            .and. create_file(test_file_names(2),       is_rsf, test_record % ip2 + 2, test_record % ip3 + 1)
        ...
        success = file_list(2) % open(test_file_names(1), options = 'R/O') .and.            &
                  file_list(3) % open(test_file_names(2), options = 'R/O')
