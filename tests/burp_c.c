...
int main (int argc, char* argv[])
{
    ...
    const int a_block_size[3] = {10, 5, 7};
    const int b_block_size[3] = { 4, 5, 7};
    const int c_block_size[3] = { 6, 5, 7};

    const int datyp_compact_uint = 2;
    ...
    *(ptle+129) =  2122;
    ...
    //////////////////////////////////////
    // Create file and initialize reports
    ...
    if (verbose > 0) printf(" *** info lbits = %d  left = %d\n",lbits,left);
    ...
    {
        int num_bits_before;
        c_mrblen(bufb, &num_bits_before, &left);
        ier = c_mrbdel(bufb,2);
        ier += c_mrblen(bufb,&lbits,&left);
    ...
    ////////////////////////
    // Test double type, replace function
    {
        uint32_t list_elem[2] = {0, 0};
        double values[12]       = { 1.0, 10.0, 100.0, 1000.0, 1e5, 1e6, 1.01, 10.02, 100.003, 1000.004, 10001.0004, 100002.00005 };
        double replacements[12] = { 2.0, 20.0, 200.0, 2000.0, 2e5, 2e6, 2.01, 20.02, 200.003, 2000.004, 20001.0004, 200002.00005 };
        double got[12];
        ier = c_mrbadd(bufc, &blkno, 2, 2, 6, 5, 12, 13, 32, &bit0, 7, list_elem, (uint32_t*)values);
        testit(ier);
        ier = c_mrbxtr(bufc, blkno, list_elem, (uint32_t*)got);
        testit(ier);

        for (int i = 0; i < 12; i++) {
            if (got[i] != values[i]) {
                printf("ERROR: Extracted value is not the same as added (%f, should be %f)\n", got[i], values[i]);
                exit(-1);
            }
        }

        ier = c_mrbrep(bufc, blkno, (uint32_t*)replacements);
        testit(ier);
        ier = c_mrbxtr(bufc, blkno, list_elem, (uint32_t*)got);
        testit(ier);

        for (int i = 0; i < 12; i++) {
            if (got[i] != replacements[i]) {
                printf("ERROR: Extracted value is not the same as replacement (%f, should be %f)\n", got[i], replacements[i]);
                exit(-1);
            }
        }

        ier = c_mrbdel(bufc, blkno);
        testit(ier);
        printf("Expecting an error: ");
        ier = c_mrbxtr(bufc, blkno, list_elem, (uint32_t*)got); // Should get "invalid block number"
        if (ier >= 0) {
            printf("ERROR: Should not have been able to extract a deleted block\n");
            exit(-1);
        }
    }
    ////////////////////////
