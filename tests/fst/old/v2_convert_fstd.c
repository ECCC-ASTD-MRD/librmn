...
typedef struct {
    uint64_t swa : 32, npas1 : 16, nk : 12, epce1 : 4;
    uint64_t ni : 16, nj : 16, nomvar : 16, typvar : 8, nBits : 8;
    uint64_t ip1 :16, ip2 : 16, ip3 : 16, epce2 : 7, dltf : 1, npas2 : 8;
    uint64_t etiq14 :32, etiq56 : 16, etiq78 : 16;
    uint64_t epce3 : 32, epce4 : 16, ig2 : 16;
    uint64_t ig3 : 16, ig4 : 16, grtyp : 8, datyp : 8, ig1 : 16;
    uint64_t date : 32, ubc : 16, deet : 16;
    uint64_t lng : 32, eof : 32;

              /****Format de bits 480-959 SEQ/SQI****/
              /* Le eof defini precedemant appartient
                 a cet interval de nombre de bits   */

    uint64_t vide1 : 32, vide2 : 32;
    uint64_t vide3 : 32, vide4 : 32;
    uint64_t epce5 : 32, epce6 : 32;
    uint64_t epce7 : 32, epce8 : 32;
    uint64_t epce9 : 32, epce10: 32;
    uint64_t epce11: 32, epce12: 32;
    uint64_t vide5 : 32, vide6 : 32;
} seq_dir_keys;


typedef struct {
   uint64_t etiqt1  : 32, etiqt2  : 32;
   uint64_t dirsiz  : 32, inuti1  : 32;
   uint64_t nutil   : 32, inuti2  : 32;
   uint64_t nbecr   : 32, inuti3  : 32;
   uint64_t nbrec   : 32, inuti4  : 32;
   uint64_t nbext   : 32, inuti5  : 32;
   uint64_t nrecup  : 32, inuti6  : 32;
   uint64_t nbeff   : 32, inuti7  : 32;
   uint64_t nbcorr  : 32, inuti8  : 32;
   uint64_t inuti9  : 32, inuti10 : 32;
   uint64_t inuti11 : 32, inuti12 : 32;
   uint64_t inut13  : 32, inuti14 : 32;
   uint64_t inuti15 : 32, inuti16 : 32;
   uint64_t inuti17 : 32, inuti18 : 32;
} stdf_struct_RND;
...
typedef struct {
    uint64_t signa : 12, ninj : 20, nscal : 16, iexpo : 12, signe : 4;
    uint64_t mantis : 32, mantirest : 16, nBits : 8, espce : 8;
} stdf_struct_data;
...
static uint64_t iexpo, iexpmin, iscal, iXmin, iTemp, result,isign,nBits, 
...
main(int argc, char *argv[]) {
    ...
    if ((fdw = open(argv[2], O_RDWR | O_CREAT,0644)) == -1) {
        fprintf(stderr, "\nErreur opening file %s\n", argv[2]);
        exit(1);
    }
...
void decompact_float(rnd_dir_keys *fkeys) {
    ...
    if (allouer < (sizeof(float) * ninj)) {
        ...
        if ((idata = (uint64_t *) malloc(allouer)) == NULL) {
            printf("\n\nmalloc : cannot allocate memory!\n\n");
            exit(1);
        }
    }
     funpacked = (uint64_t *) idata;
