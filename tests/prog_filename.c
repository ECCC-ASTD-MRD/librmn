...
int main() {
    ...
    f77name(prog_filename)(fileName, (unsigned char *)"pp", &date, &hour, &min, &sec, &npex, &npey, &num, &numlen, "", fileNameLen, 2, 0);
    int i = 0;
    while (fileName[i] != ' ') i++;
    fileName[i] = '\0';
    printf("%s\n", fileName);
    ...
    f77name(prog_filename)(fileName, (unsigned char *)"si", &date, &hour, &min, &sec, &npex, &npey, &num, &numlen, "", fileNameLen, 2, 0);
    i = 0;
    while (fileName[i] != ' ') i++;
    fileName[i] = '\0';
    printf("%s\n", fileName);
